...
function ToolBtn({ onClick, disabled, title, accent, children }: {
    ...
    return (
        <button
            ...
            disabled={disabled}
            title={title}
            className={`flex flex-col items-center justify-center gap-0.5 px-2 py-1.5 rounded-[8px] text-[10px] font-medium transition-all duration-150 active:scale-95 disabled:opacity-25 w-full border
                ${accent
                    ? 'bg-blue-600/20 border-blue-500/20 text-blue-300 hover:bg-blue-600/30 shadow-[0_0_8px_rgba(59,130,246,0.2)]'
                    : 'bg-[#111] border-[#111] text-[#555] hover:bg-[#141414] hover:text-[#e4e4e4]'
                }`}
        >
            {children}
        </button>
...
export const NoteEditor: React.FC = () => {
    ...
    const handleInsertDrawingAsImage = (dataUrl: string) => {
        ...
        editor.chain()
            ...
            .insertContent(
                ...
                src="${dataUrl}"
                alt="Dibuix"
                style="max-width:100%;border-radius:8px;margin:12px 0;border:1px solid #1a1a1a;"
            />`
            )
            .run();
    };
    ...
    const handleInsertSummary = () => {
        ...
        const summaryHtml = `<div style="background:rgba(59,130,246,0.05);padding:24px;border-radius:12px;margin:24px 0;border:1px solid rgba(59,130,246,0.15);">
        ...
        try {
            const res = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:3000/api'}/ai/chat`, {
                ...
                headers: {
                    ...
                    'Authorization': `Bearer ${localStorage.getItem('beta3m_token') || ''}`,
                },
                body: JSON.stringify({
                    messages: [{
                        role: 'user',
                        content: `A partir de este texto, extrae o sugiere una lista de tareas o puntos de acción concretos.
Responde SOLO con una lista JSON de strings, sin markdown: ["tarea 1","tarea 2",...]
Máximo 8 tareas. Texto:\n${plainText.substring(0, 3000)}`,
                    }],
                }),
            });
            ...
            const html = `<p><strong>Lista de tareas:</strong></p><ul>${tasks.map(t => `<li>&#9744; ${t}</li>`).join('')}</ul>`;
            editor?.chain().focus().insertContent('<hr />' + html).run();
            setToast('Lista de tareas generada');
            setTimeout(() => setToast(null), 3000);
        } catch {
            setError('No se pudo generar la lista de tareas.');
            setTimeout(() => setError(null), 4000);
        } finally {
            setAiLoading(false);
        }
    ...
    const handleLayerPointerDown = (e: React.PointerEvent<HTMLDivElement>) => {
        ...
        // Do NOT setPointerCapture here — deferring it lets the browser treat
        // the initial touch as a potential scroll gesture. Capture is set only
        // when the lasso threshold is crossed in handleLayerPointerMove.
        const rect = e.currentTarget.getBoundingClientRect();
        const x = e.clientX - rect.left;
        const y = e.clientY - rect.top;
        handleFloatDeselect();
        lassoRef.current = { active: false, startX: x, startY: y, points: [{ x, y }], pointerId: e.pointerId };
    };
    ...
    const handleLayerPointerMove = (e: React.PointerEvent<HTMLDivElement>) => {
        ...
        if (!lassoRef.current.active) {
            ...
            // Capture now that we're confident this is a lasso, not a scroll
            e.currentTarget.setPointerCapture(lassoRef.current.pointerId);
        }
    ...
    return (
        <div className="flex flex-col h-full bg-black relative overflow-hidden">
            ...
            <div className="px-6 py-4 flex items-center justify-between z-[100] glass border-b border-[#111]">
                <div className="flex items-center gap-4">
                    <button
                        onClick={() => setCurrentNote(null)}
                        className="p-2 rounded-xl bg-[#111] border border-[#111] hover:bg-white/10 transition-all active:scale-90"
                    >
                    ...
                    <div className="flex flex-col">
                        <div className="flex items-center gap-3">
                            ...
                            <div className="relative group">
                                <select
                                    className="appearance-none bg-[#111] border border-[#1a1a1a] rounded-lg pl-3 pr-8 py-1 text-[11px] font-black uppercase tracking-tighter text-blue-400 hover:text-white hover:bg-white/10 transition-all outline-none cursor-pointer"
            ...
            <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-[200] flex flex-col gap-2 pointer-events-none">
                {voiceError && (
                    <div className="bg-red-500 text-white rounded-xl px-6 py-3 text-sm font-bold shadow-2xl animate-fade-in-up border border-white/20 backdrop-blur-xl flex items-center gap-2">
                        <Mic size={14} /> {voiceError}
                    </div>
                )}
                {error && (
                    <div className="bg-red-500 text-white rounded-xl px-6 py-3 text-sm font-bold shadow-2xl animate-fade-in-up border border-white/20 backdrop-blur-xl flex items-center gap-2">
                ...
                {toast && (
                    <div className="bg-blue-600 text-white rounded-xl px-6 py-3 text-sm font-bold shadow-2xl animate-fade-in-up border border-white/20 backdrop-blur-xl flex items-center gap-2">
                ...
                <div className="flex flex-col items-center gap-1 py-4 px-2 glass border-r border-[#111] w-[72px] flex-shrink-0 overflow-y-auto scrollbar-hide">
                    <div className="flex gap-1 w-full">
                        <button
                            ...
                            className="flex-1 flex items-center justify-center py-1.5 rounded-[8px] bg-[#111] border border-[#111] text-[#555] hover:bg-[#141414] hover:text-[#e4e4e4] disabled:opacity-25 transition-all active:scale-95"
                        >
                            <Undo2 size={16} />
                        </button>
                        <button
                    ...
                    <div className="h-px w-10 bg-[#1a1a1a] my-1" />
                    ...
                    <ToolBtn onClick={handleGenerateTodos}
                        disabled={aiLoading || content.replace(/<[^>]*>/g, '').length < 20}
                        title="Generar lista de tareas con IA">
                        {aiLoading ? <Spinner size="sm" /> : <CheckCircle size={20} />}
                        <span className="text-[10px] uppercase tracking-tighter font-black">Tareas</span>
                    </ToolBtn>

                    <ToolBtn onClick={() => setShowFlashcards(true)} title="Modo Estudio — Flashcards IA"
                        disabled={content.replace(/<[^>]*>/g, '').trim().length < 100}>
                        <BookOpen size={20} />
                        <span className="text-[10px] uppercase tracking-tighter font-black">Estudio</span>
                    </ToolBtn>

                    <ToolBtn onClick={toggleVoice} title="Dictado por voz" accent={voiceStatus === 'listening'}>
                        {voiceStatus === 'listening' ? <MicOff size={20} /> : <Mic size={20} />}
                        <span className="text-[10px] uppercase tracking-tighter font-black">
                            {voiceStatus === 'listening' ? 'Parar' : 'Voz'}
                        </span>
                    </ToolBtn>
                ...
                <div ref={editorAreaRef} className="flex-1 overflow-y-auto overflow-x-hidden px-6 sm:px-16 py-16 note-editor-scroll relative" style={{ touchAction: inkMode ? 'none' : 'auto' }}>
                        ...
                        <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none animate-fade-in-up">
                            <div className="flex flex-col items-center gap-4 opacity-30">
                                ...
                                <div className="text-center">
                                    <p className="text-[#555] text-base font-medium">Empieza a escribir o dibuja con el lápiz</p>
                                    <p className="text-slate-600 text-sm mt-1">Escribe <kbd className="px-1.5 py-0.5 rounded bg-[#111] border border-[#1a1a1a] text-[11px] font-mono">/</kbd> para ver comandos</p>
                                </div>
                            </div>
                        </div>
                    )}
                    <div className="max-w-4xl mx-auto">
                        <input
                            ...
                            className="w-full text-6xl font-black border-none outline-none bg-transparent placeholder:text-[#333] text-white tracking-tighter leading-none mb-10 font-display focus:ring-0"
            ...
            <Modal isOpen={showDeleteModal} onClose={() => setShowDeleteModal(false)} title="Eliminar nota">
                <div className="p-2">
                    <p className="text-[#555] mb-8 font-medium text-lg leading-relaxed">¿Estás seguro de que quieres eliminar esta nota de tu biblioteca inteligente?</p>
                    <div className="flex gap-4">
                        <Button variant="secondary" className="flex-1 rounded-xl" onClick={() => setShowDeleteModal(false)}>CONSERVAR</Button>
                        <Button variant="danger" className="flex-1 rounded-xl" onClick={handleDelete}>ELIMINAR AHORA</Button>
                    </div>
                </div>
            </Modal>
            ...
            {showFlashcards && (
                <FlashcardsMode
                    noteText={editor?.getText?.() ?? content.replace(/<[^>]*>/g, ' ')}
