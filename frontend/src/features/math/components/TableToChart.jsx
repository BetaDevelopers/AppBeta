/**
 * TableToChart.jsx — Taula markdown → Gràfic Chart.js.
 * Adaptat per al backend real: POST /api/ai/table-to-chart
 * Resposta: { chartType, chartData, reasoning }
 */
...
export default function TableToChart({ onResult } = {}) {
    ...
    const chartOptions = useMemo(() => ({
        ...
        plugins: {
            ...
            tooltip: { backgroundColor: "rgba(15,10,30,0.95)", titleColor: "#a78bfa", bodyColor: "#f0e6ff", borderColor: "rgba(139,92,246,0.2)", borderWidth: 1 },
        },
        ...
        <div style={S.wrapper} id="table-analyzer-widget">
            <div style={S.header}>
                ...
                <div>
                    ...
                    <p style={S.subtitle}>Paste Markdown or CSV · AI suggests the best chart · Dynamic result</p>
                </div>
            </div>
            ...
            <div style={S.grid}>
                ...
                <div style={S.col}>
                    <div style={S.colHeader}>
                        ...
                        {result && <span style={S.typeBadge}>{result.chartType.toUpperCase()}</span>}
                    </div>
                    ...
                    <div style={S.chartBox}>
                        ...
                        {error ? (
                            ...
                            <div style={S.emptyState}>No data analyzed yet. Click the button to see the chart.</div>
                        )}
                    </div>
                    ...
                    {result && (
                        <div style={S.reasoning}>
                            ...
                            <p style={S.reasonText}><strong>AI Reasoning:</strong> {result.reasoning}</p>
                        </div>
                    )}
                </div>
            </div>
                ...
                <div style={S.footer}>
                    ...
                    <div style={S.examples}>
                        {[
                            ...
                            <button key={ex.name} style={S.exBtn} onClick={() => setInputText(ex.text)}>{ex.name}</button>
                        ))}
                    </div>
                </div>
            )}
...
const S = {
    wrapper: { fontFamily: "'Inter', sans-serif", background: "linear-gradient(135deg, #0a0618 0%, #10082a 100%)", border: "1px solid rgba(139,92,246,0.22)", borderRadius: "20px", padding: "24px", boxShadow: "0 8px 40px rgba(88,28,235,0.22), 0 2px 8px rgba(0,0,0,0.5)", color: "#e2d9f3", maxWidth: "1100px", margin: "0 auto" },
    ...
    tag: { background: "#a78bfa", color: "#fff", fontSize: "9px", fontWeight: 900, padding: "2px 6px", borderRadius: "4px", letterSpacing: "0.5px" },
    label: { fontSize: "12px", fontWeight: 700, color: "#9d8ec7", flex: 1 },
    typeBadge: { background: "rgba(16,185,129,0.15)", color: "#10b981", fontSize: "10px", fontWeight: 800, padding: "2px 8px", borderRadius: "99px", border: "1px solid rgba(16,185,129,0.3)" },
    textarea: { height: "300px", background: "rgba(0,0,0,0.3)", border: "1.5px solid rgba(139,92,246,0.2)", borderRadius: "14px", padding: "16px", fontSize: "13px", color: "#a78bfa", fontFamily: "'Fira Code', monospace", resize: "none", outline: "none" },
    btn: { background: "linear-gradient(135deg, #7c3aed, #4f46e5)", color: "#fff", border: "none", borderRadius: "10px", padding: "12px", fontSize: "14px", fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: "8px" },
    chartBox: { height: "300px", background: "rgba(255,255,255,0.02)", border: "1.5px solid rgba(139,92,246,0.15)", borderRadius: "14px", display: "flex", alignItems: "center", justifyContent: "center", position: "relative", padding: "20px", overflow: "hidden" },
    chartInner: { width: "100%", height: "100%" },
    emptyState: { color: "#5a507a", fontSize: "14px", fontWeight: 500, fontStyle: "italic" },
    loadingOverlay: { position: "absolute", inset: 0, background: "rgba(10,6,24,0.7)", zIndex: 10, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: "12px" },
    ...
    reasoning: { marginTop: "12px", background: "rgba(124,58,237,0.06)", border: "1px solid rgba(124,58,237,0.2)", borderRadius: "10px", padding: "12px 14px", display: "flex", gap: "10px", alignItems: "flex-start" },
    ...
    exBtn: { background: "rgba(139,92,246,0.1)", border: "1px solid rgba(139,92,246,0.25)", borderRadius: "6px", padding: "4px 10px", color: "#a78bfa", fontSize: "11px", cursor: "pointer" },
};
