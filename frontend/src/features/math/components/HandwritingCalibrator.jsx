/**
 * HandwritingCalibrator.jsx — Calibrar estil escriptura.
 * Adaptat per al backend real: POST /api/ai/calibrate
 * Resposta: { avgCharWidth, avgCharHeight, slantAngle, suggestedFontSize, normalizationMatrix }
 */

import { useRef, useState } from "react";
import { calibrateHandwriting } from "../../../api/mathApi";

export default function HandwritingCalibrator() {
    ...
    async function calibrate(sample) {
        ...
        setError(null);
        try {
            const data = await calibrateHandwriting(sample);
            setProfile(data);
        } catch (e) {
            setError(e.message);
        } finally {
