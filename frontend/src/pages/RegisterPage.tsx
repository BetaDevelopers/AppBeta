        ...
        <div className="min-h-screen bg-[#0a0f1e] flex items-center justify-center p-4">
            <div className="w-full max-w-[440px] bg-[#0f172a] rounded-[32px] shadow-2xl border border-white/5 p-8 sm:p-12">
                ...
                <form onSubmit={handleRegister} className="space-y-6">
                                ...
                                <div className="flex gap-1">
                                    {[1, 2, 3, 4].map(i => {
                                        const s = (() => {
                                            ...
                                            let sc = 1;
                                            if (/[0-9]/.test(password)) sc++;
                                            if (/[A-Z]/.test(password)) sc++;
                                            if (/[^A-Za-z0-9]/.test(password)) sc++;
                                            return sc;
                                        })();
                                        const color = s <= 1 ? '#ef4444' : s === 2 ? '#eab308' : s === 3 ? '#84cc16' : '#22c55e';
                                        return (
                                            <div key={i} className="h-1 flex-1 rounded-full transition-all duration-300"
                                                style={{ backgroundColor: i <= s ? color : '#1a1a1a' }} />
                                        );
                                    })}
                                </div>
                                <p className="text-xs" style={{ color: password.length < 8 ? '#ef4444' : '#6b7280' }}>
                                    {password.length < 8 ? `Faltan ${8 - password.length} caracteres` : 'Mínimo 8 caracteres'}
                                </p>
                            </div>
                        )}
                    </div>
                    ...
                    <div className="pt-2">
                        <Button type="submit" loading={loading} disabled={password.length < 8} className="w-full gap-3 text-lg h-14">
